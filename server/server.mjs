...
// Fallback GPU solo cuando nvidia-smi no esta activo
repeat(async () => {
 ...
 state.gpuSource = 'Sistema operativo · informacion basica'
 mark('gpu')
...
if (process.platform === 'win32') {
 let netChild = null
 let netBuf = ''
 // Script PS que corre en bucle: emite una linea JSON por adaptador activo cada 2s
 const NET_PS = [
 '$p=@{}',
 'while($true){',
 ...
 if (!obj.up) continue
 // Preferir adaptador con trafico; si ninguno tiene, al menos fijar el nombre
...
async function readSystemDetails() {
 ...
 // Extraer el JSON desde la ultima linea que empiece con '{' — robusto contra
 ...
 if (!jsonLine) throw new Error('system-info.ps1 no produjo JSON valido.')
 ...
 if (!hasInventory) throw new Error('Windows restringio el inventario WMI.')
 details.cpu ||= {}
...
// Recibe datos del frontend, agrega hostname + machineId y reenvia al ranking server
...
app.post('/api/stress/start', async (req, res) => {
 if (state.session?.active) return res.status(409).json({ error: 'Ya existe una prueba en ejecuci-n.' })
 const requested = req.body || {}
 ...
 return res.status(412).json({ error: 'No hay una temperatura CPU real y reciente. La prueba se bloqueo por seguridad.' })
...
async function performUpdateCheck(force = false) {
 ...
 updateState = { ...updateState, currentVersion: current, checkedAt: Date.now(), checking: false, hasUpdate: false, latestVersion: current, downloadUrl: null, changelog: '', error: 'Las actualizaciones automaticas no estan disponibles en este sistema.' }
 return updateState
 }
...
app.post('/api/update/install', (req, res) => {
 if (process.platform !== 'win32') return res.status(400).json({ error: 'La instalacion automatica de actualizaciones esta disponible actualmente solo en Windows.' })
...
if (process.platform === 'win32') {
 ...
 currentVersion: appManifest.version,
 latestVersion: appManifest.version,
 error: 'Las actualizaciones automaticas no estan disponibles en este sistema.'
 }
}
// ─────────────────────────────────────────────────────────────────────────────
