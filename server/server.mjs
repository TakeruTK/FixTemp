...
app.post('/api/update/install', (req, res) => {
  ...
  setTimeout(async () => {
    ...
    const script = [
      ...
      `$form.Show()`,
      `function Set-UpdateStatus([string]$message, [string]$info = '') { $label.Text = $message; if ($info) { $detail.Text = $info }; [System.Windows.Forms.Application]::DoEvents(); Write-FixTempLog $message }`,
