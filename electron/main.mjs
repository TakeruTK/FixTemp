...
async function createWindow() {
 ...
 await mainWindow.loadURL(bootHtml('Estamos iniciando el servicio local y preparando la interfaz. Esto puede tardar unos segundos segun el equipo.'))
 mainWindow.show()
 ...
 await mainWindow.loadURL(bootHtml(`La interfaz aun no responde (${errorCode}: ${errorDescription}). Reintentando automaticamente...`))
 ...
 if (!ready) {
 await mainWindow.loadURL(bootHtml('El servicio interno tardo demasiado en responder. Cierra esta ventana y vuelve a intentar. Si persiste, avisame para dejarlo corregido en la beta.'))
 return
 }
