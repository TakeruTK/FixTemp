...
const copy = {
  es: {
    ...
    loadingTitle: 'Estamos reuniendo toda la ficha técnica del equipo',
    loadingBody: 'Esta vista tarda más que las demás porque también cruza datos avanzados de Windows, DirectX, buses y almacenamiento. Mientras siga este panel, el inventario todavía se está completando.',
    waitFull: 'El usuario puede esperar aquí hasta que termine la carga completa de “Mi equipo”.',
    loadingSteps: [
      ...
      'Relacionando discos físicos con volúmenes y firmware'
    ],
    ...
    memory: 'Memoria RAM',
    physicalStorage: 'Almacenamiento físico',
    ...
    physicalProcessors: 'Procesadores físicos',
    baseClock: 'Reloj base',
    maxClock: 'Reloj máximo',
    cache: 'Cache L2 / L3',
    virtualization: 'Virtualización visible',
    tdp: 'TDP de referencia',
    ramCapacity: 'Capacidad',
    ramConfiguredSpeed: 'Velocidad configurada',
    ramMaxSpeed: 'Velocidad máxima (rated)',
    ...
    gpuIndex: 'GPU',
    maxBoardCapacity: 'Capacidad máxima placa',
    ...
    free: 'libres',
    transferNote: 'La medición escribe y lee un archivo temporal verificado en el volumen seleccionado. Así vemos caudal real sin depender de otra utilidad externa.',
    logicalCapacity: 'Capacidad lógica detectada',
    measure: 'Medir',
    measuring: 'Midiendo…',
    write: 'Escritura',
    read: 'Lectura',
    ...
    waitingVolumes: 'Esperando volúmenes montados para poder medir transferencia…',
    detectingAudio: 'Detectando dispositivos de audio…',
    adapter: 'ADAPTADOR',
    speedNotReported: 'Velocidad no reportada',
    activeAdapter: 'Adaptador activo',
    waitingDisplays: 'Esperando información de pantallas…',
    ...
    battery: 'Batería · capacidad/ciclos',
    ...
    <div className={`hardware-grid ${loading ? 'loading-grid' : ''}`}>
      ...
      <section className="hardware-card">
        ...
        {(details?.graphics?.controllers?.length ? details.graphics.controllers : [{ model: data.gpu.model, vendor: data.gpu.vendor, vram: data.gpu.memoryTotal }]).map((gpu, index) => {
          // Win32_VideoController.AdapterRAM está limitado a 32-bit → máx 4095 MB para GPUs con >4 GB VRAM
          // Si WMI da ≤4095 y nvidia-smi reporta más, usar nvidia-smi (memoryTotal es en MB)
