...
interface MetricCardProps {
  ...
  stats: { label: string; value: ReactNode; progress?: number | null }[]; badge?: string
}
...
export function MetricCard({ title, subtitle, value, tone = 'cyan', history, stats, badge }: MetricCardProps) {
  ...
  return (
    <section className={`metric-card metric-card--${tone}`}>
      <div className="card-heading"><div><p className="eyebrow">{subtitle}</p><h2>{title}</h2></div>{badge && <span className="chip">{badge}</span>}</div>
