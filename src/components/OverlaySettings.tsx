...
const copy = {
  es: {
    readError: 'No se pudo leer la configuraciÃ³n.',
    saveError: 'No se pudo guardar',
    eyebrow: 'MONITOREO EN JUEGOS Y PROGRAMAS',
    title: 'Overlay en pantalla',
    description: 'TelemetrÃ­a compacta, siempre visible y sin capturar clics ni teclado.',
    disable: 'Desactivar overlay',
    enable: 'Activar overlay',
    visibleMetrics: 'MÃ‰TRICAS VISIBLES',
    appearance: 'APARIENCIA Y POSICIÃ“N',
    position: 'PosiciÃ³n',
    layout: 'DiseÃ±o',
    compact: 'Compacto',
    normal: 'Normal',
    ...
    fpsInfo: 'FixTemp no inventa FPS: quedarÃ¡ â€œâ€”â€ hasta conectar un proveedor de cuadros compatible. Uso, VRAM, temperaturas y watts sÃ­ provienen de los sensores actuales.',
    noData: 'â€”',
    cpu: 'Uso de CPU',
    cpuDetail: 'Carga total del procesador',
    gpu: 'Uso de GPU',
    gpuDetail: 'Carga del adaptador grÃ¡fico',
    ram: 'Uso de RAM',
    ramDetail: 'Porcentaje de memoria del sistema',
    vram: 'Uso de VRAM',
    vramDetail: 'Memoria grÃ¡fica dedicada',
    temperatures: 'Temperaturas',
    temperaturesDetail: 'CPU y GPU cuando existe sensor',
    power: 'Consumo en watts',
    powerDetail: 'SÃ³lo lecturas fÃ­sicas disponibles',
    fps: 'FPS del juego',
    fpsDetail: 'Se muestra sÃ³lo con proveedor compatible'
  },
  en: {
    ...
    fpsInfo: 'FixTemp does not invent FPS: it stays as â€œâ€”â€ until a compatible frame provider is connected. Usage, VRAM, temperatures, and watts do come from current sensors.',
    noData: 'â€”',
    cpu: 'CPU usage',
  ...
  'zh-CN': {
    readError: 'æ— æ³•è¯»å–é…ç½®ã€‚',
    saveError: 'æ— æ³•ä¿å­˜',
    eyebrow: 'æ¸¸æˆä¸Žç¨‹åºç›‘æŽ§',
    title: 'å±å¹•æ‚¬æµ®å±‚',
    description: 'ç´§å‡‘åž‹é¥æµ‹ä¿¡æ¯ï¼Œå§‹ç»ˆå¯è§ï¼Œå¹¶ä¸”ä¸ä¼šæ‹¦æˆªé¼ æ ‡æˆ–é”®ç›˜æ“ä½œã€‚',
    disable: 'å…³é—­æ‚¬æµ®å±‚',
    enable: 'å¼€å¯æ‚¬æµ®å±‚',
    ...
    bottomLeft: 'å·¦ä¸‹',
    bottomRight: 'å³ä¸‹',
    windowed: 'é€‚ç”¨äºŽçª—å£åŒ–æˆ–æ— è¾¹æ¡†çª—å£æ¸¸æˆã€‚ç‹¬å å…¨å±æ¨¡å¼å¯èƒ½ä¼šé˜»æ­¢å¤–éƒ¨æ‚¬æµ®å±‚ã€‚',
    fpsInfo: 'FixTemp ä¸ä¼šè™šæž„ FPSï¼šåœ¨è¿žæŽ¥å…¼å®¹çš„å¸§çŽ‡æä¾›ç¨‹åºä¹‹å‰ä¼šæ˜¾ç¤ºä¸º â€œâ€”â€ã€‚ä½¿ç”¨çŽ‡ã€VRAMã€æ¸©åº¦å’ŒåŠŸè€—æ¥è‡ªå½“å‰ä¼ æ„Ÿå™¨ã€‚',
    noData: 'â€”',
    cpu: 'CPU ä½¿ç”¨çŽ‡',
    ...
    power: 'åŠŸè€—ï¼ˆç“¦ï¼‰',
    powerDetail: 'ä»…æ˜¾ç¤ºå®žé™…ç‰©ç†è¯»æ•°',
    fps: 'æ¸¸æˆ FPS',
...
export function OverlaySettings({ data }: { data: Metrics }) {
  ...
  return <div className="overlay-page">
    ...
    <div className="overlay-settings-grid"><section className="overlay-card"><p className="section-label">{text.visibleMetrics}</p><div className="overlay-toggles">{options.map(option => <button key={option.key} className={config.metrics[option.key] ? 'on' : ''} onClick={() => updateMetric(option.key)}><span><b>{option.label}</b><small>{option.detail}</small></span><i/></button>)}</div></section>
      ...
      <section className="overlay-card overlay-preview-card"><p className="section-label">{text.preview}</p><div className="settings-preview" style={{ opacity: config.opacity / 100 }}><div><i/>FIXTEMP <span>LIVE</span></div><section><p><span>CPU</span><b>{data.cpu.load}%</b></p><p><span>GPU</span><b>{data.gpu.load}%</b></p><p><span>RAM</span><b>{data.memory.load}%</b></p><p><span>CPU TEMP</span><b>{data.cpu.temperature === null ? text.noData : `${data.cpu.temperature}Â°C`}</b></p></section></div><div className="overlay-info"><MonitorUp size={17}/><span>{text.windowed}</span></div><div className="overlay-info"><Info size={17}/><span>{text.fpsInfo}</span></div></section>
    </div>
  </div>
}
