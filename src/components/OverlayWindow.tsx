...
const labels = {
  ...
  'zh-CN': { cpuTemp: 'CPU æ¸©åº¦', gpuTemp: 'GPU æ¸©åº¦', cpuW: 'CPU åŠŸè€—', gpuW: 'GPU åŠŸè€—' }
} as const
...
function OverlayMetricsPanel({ config }: { config: OverlayConfig }) {
  ...
  if (config.metrics.vram) items.push({ key: 'vram', label: 'VRAM', value: data.gpu.memoryTotal > 0 ? `${(data.gpu.memoryUsed / 1024).toFixed(1)} GB` : 'â€”' })
  if (config.metrics.temperatures) {
    items.push({ key: 'cpu-temp', label: text.cpuTemp, value: data.cpu.temperature === null ? 'â€”' : `${data.cpu.temperature}Â°C`, tone: 'warm' })
    items.push({ key: 'gpu-temp', label: text.gpuTemp, value: data.gpu.temperature === null ? 'â€”' : `${data.gpu.temperature}Â°C`, tone: 'warm' })
  }
  if (config.metrics.power) {
    items.push({ key: 'cpu-power', label: text.cpuW, value: data.cpu.power === null ? 'â€”' : `${data.cpu.power} W` })
    items.push({ key: 'gpu-power', label: text.gpuW, value: data.gpu.power === null ? 'â€”' : `${data.gpu.power} W` })
  }
  if (config.metrics.fps) items.push({ key: 'fps', label: 'FPS', value: data.fps === null || data.fps === undefined ? 'â€”' : `${data.fps}`, tone: 'lime' })
