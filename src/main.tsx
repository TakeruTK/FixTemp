  ...
  root.innerHTML = `
    <div style="min-height:100vh;display:grid;place-items:center;background:#080b0d;color:#e6ecee;font-family:Segoe UI,Arial,sans-serif;padding:24px">
      <div style="width:min(560px,92vw);padding:30px;border:1px solid #1d2529;background:linear-gradient(145deg,#12181b,#0f1417);box-shadow:0 18px 50px rgba(0,0,0,.35)">
        <p style="margin:0 0 10px;color:#42e6f5;font:600 12px monospace;letter-spacing:.14em;text-transform:uppercase">FixTemp</p>
        <h1 style="margin:0 0 12px;font-size:28px">La interfaz encontrÃ³ un problema</h1>
        <p style="margin:0 0 10px;color:#b9c6cb;line-height:1.7">${message}</p>
        <p style="margin:0;color:#8b989e;line-height:1.7">Cierra y vuelve a abrir la aplicaciÃ³n. Si vuelve a pasar, esta beta necesita correcciÃ³n antes de seguir probando en mÃ¡s equipos.</p>
      </div>
    </div>
  `
}
...
class AppCrashBoundary extends Component<{ children: ReactNode }, { hasError: boolean }> {
  ...
  componentDidCatch(error: unknown) {
    ...
    console.error('FixTemp renderer error:', error)
...
async function prepareDesktopRuntime() {
  ...
  try {
    ...
    console.warn('No se pudo limpiar la cachÃ© local de escritorio:', error)
  }
}
